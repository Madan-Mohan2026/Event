...
export async function handleRoute(state) {
  try {
    ...
    console.error('❌ [ROUTER CRASH PREVENTED]: Error rendering route:', err);
    const app = document.getElementById('app');
    if (app) {
      app.innerHTML = `
        <div style="min-height: 80vh; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px; text-align: center; font-family: system-ui, sans-serif;">
          <div style="font-size: 48px; margin-bottom: 12px;">⚠️</div>
          <h2 style="color: #0f172a; font-size: 22px; font-weight: 800; margin-bottom: 8px;">Unexpected Navigation Error</h2>
          <p style="color: #64748b; font-size: 14px; max-width: 480px; margin-bottom: 20px;">${err.message || 'An error occurred while loading this page.'}</p>
          <div style="display:flex; gap:12px;">
            <button onclick="window.location.hash='#events'" style="background: #4f46e5; color: white; border: none; padding: 10px 20px; border-radius: 10px; font-weight: 700; cursor: pointer; box-shadow: 0 4px 14px rgba(79,70,229,0.3);">
              Return to Events
            </button>
            <button onclick="window.location.reload()" style="background: #f1f5f9; color: #334155; border: 1px solid #cbd5e1; padding: 10px 20px; border-radius: 10px; font-weight: 700; cursor: pointer;">
              Reload Page
            </button>
          </div>
        </div>
